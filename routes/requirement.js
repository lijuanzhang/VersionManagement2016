    ...
    TaskAdmin.findAllTask(params, function (msg, reqs, count) {
        if (msg == "success") {
            PermissionAdmin.getMenus(params, function (msg, menus) {
                ...
                return res.render("requirements/reqIndex", {
                    title: '设计需求子系统', pageName: 'tasks', tasks: reqs, count: count,
                    user: params.user,
...
function findToDealTask(req, res, startNum) {
    ...
    PermissionAdmin.getMenus(params, function (msg, menus) {
        ...
        TaskAdmin.findDealTask(params, function (msg, result, count, curDealPage) {
            ...
            var totalDealPage = parseInt((count - 1) / 30 + 1);
            return res.render("requirements/reqIndex", {
                title: '设计需求子系统', pageName: 'tasks', tasks: result, count: count,
                user: user,
...
function findReqAtta(req, res, params, curPage) {
    ...
    PermissionAdmin.getMenus(params, function (msg, menus) {
        ...
        TaskAdmin.searchAtta(params, function (msg, result, count) {
            ...
            var totalDealPage = parseInt((count - 1) / 30 + 1);
            return res.render("requirements/reqIndex", {
                title: '设计需求子系统', pageName: 'attas', attas: result, count: count,
                user: user,
                curDealPage: curPage,
                totalDealPage: totalDealPage,
                menus: menus,
    ...
    Project.getUserProject(params, function (msg, result) {
        if (msg == "success") {
            ...
            PermissionAdmin.getButtons(getButtonParams, function (msg, buttons) {
                ...
                res.render('requirements/reqProcess', {
                    allInfo: allInfo,
                    infoDivs: "",
                    stateId: 0,
                    buttons: buttons,
    ...
    TaskAdmin.getTaskInfo(params, function (msg, allInfo) {
        if (msg == "success") {
            PermissionAdmin.getButtons(params, function (msg, buttons) {
                ...
                res.render('requirements/reqProcess', {
                    reqProcessStepId: params.reqProcessStepId,
                    allInfo: allInfo,
                    infoDivs: "",
                    stateId: allInfo.taskInfo.stateId,
                    buttons: buttons,
                    projectId: allInfo.taskInfo.projectId,
                    processStepId: allInfo.taskInfo.processStepId,
                    taskInfo: allInfo.taskInfo,
                    reqId: allInfo.taskInfo.reqId
                });
            })
...
router.get('/findReqs', function (req, res, next) {
    ...
    Project.getUserProject(params, function (msg, result) {
        if (msg == "success") {
            res.render('requirements/findAllrequirement', {
                url: "/req/findReqs",
...
router.post('/nextDealer', function (req, res, next) {
    ...
    params.userId = user.userId;
    var dealerData = {
        userName: params.dealer, processStepId: params.processStepId,
...
router.get('/findAttasPage', function (req, res) {
    ...
    Project.getUserProject(params, function (msg, result) {
        if (msg == "success") {
            var allProject = result;
            res.render('requirements/findAttas', {
                url: "/req/findAllAttas", modalTitle: "查找附件",
                projects: allProject
            });
        }
        else {
