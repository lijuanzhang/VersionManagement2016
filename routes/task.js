    ...
    dao.searchAllProject(cookieUser.userId, function (msg, result) {
        if (msg == "success") {
            ...
            res.render('submitApply', {
                project: allProject,
                requirements: {reqId: reqId, reqName: reqName},
                isReq: true
            });//isReq:判断是否是从设计系统直接进入
        }
        else {
            return req.session.error = "查找待处理需求失败，请联系管理员";
        }
...
router.post('/addBugTask', function (req, res) {
    ...
    dao.addBugTask({
        ...
        desc: taskDetails,
        newFiles: taskNewFiles,
        modFiles: taskModFiles,
        delFiles: taskDelFiles,
        bugId: bugId
    }, function (msg, taskId, taskCode) {
