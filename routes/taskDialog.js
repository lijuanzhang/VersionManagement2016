...
var showFileList = function (taskId) {
    taskDao.getFileList(taskId, function (msg, result) {
        if (msg === "success") {
            ...
            res.render("showFileList", {
                fileCount: result.length,
                modFiles: modFilesDiv,
                newFiles: newFilesDiv,
                delFiles: delFilesDiv
            })
        }
    });
}
...
function getTaskInfos(params, callback) {
    ApplyOrder.getTaskInfos(params, function (msg, taskInfos) {
        if (msg == "success") {
            ...
            var infos = {
                task: taskInfos.taskInfo, modFileList: modFileList, addFileList: addFileList, delFileList: delFileList,
...
function openTaskDialog(stepName, req, res) {
    ...
    if (stepName == "submitToDev") {
        var newParams = {
            userId: userId,
            dealer: req.params.dealerName,
            creater: params.createName,
        ...
        hasPermissionOfDealer(newParams, function (isDealer) {
            if (isDealer) {
                ...
                getApplyOrder(newParmas, function (msg, orderInfo) {
                    ...
                    res.render(stepName, {
                        task: orderInfo.taskInfo[0],
                        applyInfo: orderInfo.applyInfo,
                        taskAttas: orderInfo.taskAttas,
                        operator: true
                    });
                })
            }
                ...
                getTaskInfos(newParams, function (msg_taskInfo, taskInfos) {//获取变更单的基本信息
                    ...
                    res.render('taskInfo', {
                        task: taskInfos.task[0],
                        addFileList: taskInfos.addFileList,
                        modifyFileList: taskInfos.modifyFileList,
                        delFileList: taskInfos.delFileList,
                        attaFile: taskInfos.attas
                    });
                });
            }
            ;
